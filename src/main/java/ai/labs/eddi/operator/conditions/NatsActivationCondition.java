...
/**
 * Activates NATS DRs only when spec.messaging.type=nats and managed.enabled=true.
 * Uses HasMetadata so this condition works for StatefulSet, Service, etc.
 */
public class NatsActivationCondition implements Condition<HasMetadata, EddiResource> {

    @Override
    public boolean isMet(DependentResource<HasMetadata, EddiResource> dependentResource,
                          EddiResource eddi,
                          Context<EddiResource> context) {
        return Defaults.isManagedNats(eddi.getSpec());
    }
}
